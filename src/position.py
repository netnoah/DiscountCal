...
def load_positions(filepath: str) -> list[dict]:
    ...
    for idx, row in enumerate(rows):
        ...
        contract, buy_date_str, buy_price, base_price, lots, sold_str = row[:6]
...
def save_position_result(filepath: str, row_index: int, result: dict) -> None:
    """Write all calculated fields back to Excel.

    Columns: 7=当前价, 8=当前贴水, 9=已吃贴水, 10=收敛比例, 11=贴水年化, 12=合约浮盈
    """
    if not Path(filepath).exists():
        return
    wb = openpyxl.load_workbook(filepath)
    ws = wb.active
    ws.cell(row=row_index, column=7, value=result["current_price"])
    ws.cell(row=row_index, column=8, value=result["current_basis"])
    ws.cell(row=row_index, column=9, value=result["captured_basis"])
    conv = result["convergence_pct"]
    ws.cell(row=row_index, column=10, value=f"{conv:.1f}%" if conv is not None else None)
    ann = result["annualized_return"]
    ws.cell(row=row_index, column=11, value=f"{ann:.2f}%" if ann is not None else None)
    ws.cell(row=row_index, column=12, value=result["pnl"])
    wb.save(filepath)
    wb.close()
